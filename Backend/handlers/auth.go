...
func GithubAuth(c *gin.Context) {
	...
	if clientID == "" || clientSecret == "" {
		c.JSON(http.StatusInternalServerError, models.NewErrorResponse("GitHub configuration missing", nil))
		return
	}

	// 1. Exchange code for access token
	accessToken, err := getGithubAccessToken(clientID, clientSecret, req.Code)
	if err != nil {
		c.JSON(http.StatusUnauthorized, models.NewErrorResponse("failed to get github access token", nil))
		return
	}
	...
	if err != nil {
		c.JSON(http.StatusInternalServerError, models.NewErrorResponse("failed to get github user info", nil))
		return
	}

	// 3. Fetch User Email (GitHub often needs a separate call)
	email, err := getGithubEmail(accessToken)
	if err != nil || email == "" {
		c.JSON(http.StatusInternalServerError, models.NewErrorResponse("failed to get github email", nil))
		return
	}
	...
	c.JSON(http.StatusOK, models.NewSuccessResponse("github login successful", gin.H{
		"token": token,
		"user": gin.H{
			"id":         userID,
			"email":      user.Email,
...
func getGithubAccessToken(clientID, clientSecret, code string) (string, error) {
	v := url.Values{}
	v.Set("client_id", clientID)
	v.Set("client_secret", clientSecret)
	v.Set("code", code)

	req, _ := http.NewRequest("POST", "https://github.com/login/oauth/access_token", strings.NewReader(v.Encode()))
